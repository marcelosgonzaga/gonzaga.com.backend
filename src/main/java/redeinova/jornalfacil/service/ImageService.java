...
public class ImageService {
    ...
    public ImageService(ImageConverter imageConverter) {
    ...
    private void desenharTemaFundo(Graphics2D g2d, BufferedImage bufferedImage, Projeto projeto) throws IOException {
        if (projeto.getTema() != null && projeto.getTema().getCaminhoImagem() != null) {
            try {
                ...
                BufferedImage temaImage;
                if (imageConverter.isWebpFormat(temaPath)) {
                    byte[] imageBytes = imageConverter.convertWebpToPng(temaPath);
                    temaImage = ImageIO.read(new java.io.ByteArrayInputStream(imageBytes));
                } else {
                    temaImage = ImageIO.read(new File(temaPath));
                }
        ...
        if (produto.getCaminhoImagem() != null) {
            try {
                ...
                if (imageConverter.isWebpFormat(imagePath)) {
                    ...
                    productImage = ImageIO.read(new java.io.ByteArrayInputStream(imageBytes));
                } else {
                    productImage = ImageIO.read(new File(imagePath));
                }
