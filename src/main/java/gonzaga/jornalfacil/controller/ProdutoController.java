...
public class ProdutoController {
    ...
    public ResponseEntity<ProdutoResponse> atualizarPrecos(@PathVariable Long id, @RequestBody Map<String, Object> dados) {
        try {
            Produto produto = produtoService.buscarPorId(id);

            if (dados.containsKey("precoDe")) {
                produto.setPrecoDe(new BigDecimal(dados.get("precoDe").toString()));
            }

            if (dados.containsKey("precoPor")) {
                produto.setPrecoPor(new BigDecimal(dados.get("precoPor").toString()));
            }

            // ✅ NOVOS CAMPOS (opcionais)
            if (dados.containsKey("informacoesObrigatorias")) {
                produto.setInformacoesObrigatorias((String) dados.get("informacoesObrigatorias"));
            }

            if (dados.containsKey("textoLegal")) {
                produto.setTextoLegal((String) dados.get("textoLegal"));
            }
            ...
            logger.error("Erro ao atualizar produto {}: {}", id, e.getMessage());
    ...
    public ResponseEntity<List<ProdutoResponse>> atualizarPrecosEmLote(@RequestBody List<Map<String, Object>> produtosComPrecos) {
        try {
            ...
            for (Map<String, Object> produtoPreco : produtosComPrecos) {
                ...
                // ✅ CORREÇÃO: Processar campo isento
                ...
                // ✅ CORREÇÃO: Definir campo isento
                produto.setIsento(isIsento);
    ...
    private ProdutoResponse toResponse(Produto produto) {
        return ProdutoResponse.builder()
                ...
                // ✅ NOVOS CAMPOS
                .informacoesObrigatorias(produto.getInformacoesObrigatorias())
                .textoLegal(produto.getTextoLegal())
                .build();
    }
