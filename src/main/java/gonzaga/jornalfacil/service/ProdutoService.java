...
public class ProdutoService {
    ...
    public Produto criarProduto(NovoProdutoDTO dto) {
        ...
        // ✅ NOVOS CAMPOS
        produto.setInformacoesObrigatorias(dto.getInformacoesObrigatorias());
        produto.setTextoLegal(dto.getTextoLegal());
    ...
    public Produto atualizarProduto(Produto produto) {
        logger.info("Atualizando produto ID: {} - PrecoDe: {}, PrecoPor: {}, Classificacao: {}, Isento: {}",
                produto.getId(), produto.getPrecoDe(), produto.getPrecoPor(),
                produto.getClassificacao(), produto.isIsento());
        ...
        // ✅ CORREÇÃO: Lógica para medicamentos isentos
        if (produto.getClassificacao() == ClassificacaoProduto.MEDICAMENTO) {
            if (produto.isIsento()) {
                // Para medicamentos isentos, precoDe deve ser null
                logger.info("Produto ID {} marcado como ISENTO - definindo precoDe como null", produto.getId());
                produto.setPrecoDe(null);
            } else {
                // Para medicamentos não isentos, validar precoDe
                if (produto.getPrecoDe() == null) {
                    throw new InvalidProductDataException("PMC é obrigatório para medicamentos não isentos");
                }

                if (produto.getPrecoDe().compareTo(BigDecimal.ZERO) < 0) {
                    throw new InvalidProductDataException("PMC não pode ser negativo");
                }
        ...
        logger.info("Produto salvo - ID: {} - PrecoDe: {}, PrecoPor: {}, Isento: {}, InfoObrig: {}, TextoLegal: {}",
                produtoSalvo.getId(), produtoSalvo.getPrecoDe(),
                produtoSalvo.getPrecoPor(), produtoSalvo.isIsento(),
                produtoSalvo.getInformacoesObrigatorias() != null ? "OK" : "null",
                produtoSalvo.getTextoLegal() != null ? "OK" : "null");
...
//
//        Produto produtoSalvo = produtoRepository.save(produto);
//        logger.info("Produto salvo - ID: {} - PrecoDe: {}, PrecoPor: {}, Isento: {}",
//                produtoSalvo.getId(), produtoSalvo.getPrecoDe(),
//                produtoSalvo.getPrecoPor(), produtoSalvo.isIsento());
//
//        return produtoSalvo;
